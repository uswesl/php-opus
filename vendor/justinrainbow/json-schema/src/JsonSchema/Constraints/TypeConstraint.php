...
class TypeConstraint extends Constraint
{
    ...
    static $wording = array(
        'integer' => 'um inteiro (integer)',
        'number'  => 'um numero (number)',
        'boolean' => 'um booleano (boolean)',
        'object'  => 'um objeto (object)',
        'array'   => 'um vetor (array)',
        'string'  => 'uma string',
        'null'    => 'um nulo (null)',
        ...
        if ($isValid === false) {
            if (!isset(self::$wording[$type])) {
                throw new StandardUnexpectedValueException(
                    sprintf(
                        "Sem escrita disponivel para %s, escritas esperadas sao: [%s]",
                        var_export($type, true),
                        implode(', ', array_filter(self::$wording)))
                );
            }
            $this->addError($path, ucwords(gettype($value)) . " encontrado, porém o esquema define como " . self::$wording[$type], 'type');
        }
    }
    ...
    protected function validateType($value, $type)
    {
        ...
        throw new InvalidArgumentException((is_object($value) ? 'object' : $value) . ' eh um tipo invalido para ' . $type);
    }
}
