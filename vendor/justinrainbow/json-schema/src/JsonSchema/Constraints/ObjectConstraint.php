    ...
    public function validatePatternProperties($element, $path, $patternProperties)
    {
        ...
        foreach ($patternProperties as $pregex => $schema) {
            ...
            if (@preg_match($delimiter. $pregex . $delimiter, '') === false) {
                $this->addError($path, 'O padrao da expressao regular "' . $pregex . '" esta invalido', 'pregex', array('pregex' => $pregex,));
                continue;
            }
    ...
    public function validateElement($element, $matches, $objectDefinition = null, $path = null, $additionalProp = null)
    {
        ...
        foreach ($element as $i => $value) {
            ...
            if (!in_array($i, $matches) && $additionalProp === false && $this->inlineSchemaProperty !== $i && !$definition) {
                $this->addError($path, "O elemento " . $i . " nao esta definido e a definicao existente nao permite elementos adicionais", 'additionalProp');
            }
            ...
            if ($require && !$this->getProperty($element, $require)) {
                $this->addError($path, "A presenca do elemento " . $i . " requer que " . $require . " tambem esteja presente", 'requires');
            }
    ...
    protected function validateMinMaxConstraint($element, $objectDefinition, $path) {
        ...
        if (isset($objectDefinition->minProperties) && !is_object($objectDefinition->minProperties)) {
            if (count(get_object_vars($element)) < $objectDefinition->minProperties) {
                $this->addError($path, "Precisa contem um minimo de " . $objectDefinition->minProperties . " propriedades", 'minProperties', array('minProperties' => $objectDefinition->minProperties,));
            }
        }
        ...
        if (isset($objectDefinition->maxProperties) && !is_object($objectDefinition->maxProperties)) {
            if (count(get_object_vars($element)) > $objectDefinition->maxProperties) {
                $this->addError($path, "Nao deve conter mais de " . $objectDefinition->maxProperties . " propriedades", 'maxProperties', array('maxProperties' => $objectDefinition->maxProperties,));
            }
        }
    }
}
