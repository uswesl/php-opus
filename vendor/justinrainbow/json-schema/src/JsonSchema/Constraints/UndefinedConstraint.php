...
class UndefinedConstraint extends Constraint
{
    ...
    protected function validateCommonProperties($value, $schema = null, $path = null, $i = "")
    {
            ...
            if (!($value instanceof UndefinedConstraint) && isset($schema->required) && is_array($schema->required) ) {
                ...
                foreach ($schema->required as $required) {
                    if (!property_exists($value, $required)) {
                        $this->addError((!$path) ? $required : "$path.$required", "O elemento " . $required . " é requerido", 'requerido');
                    }
                }
                ...
                if ( $schema->required && $value instanceof UndefinedConstraint) {
                    $this->addError($path, "Não existe e é requerido", 'requerido');
                }
            }
        }
        ...
        if (isset($schema->disallow)) {
            ...
            if (count($this->getErrors()) == count($initErrors)) {
                $this->addError($path, "Valor nao permitido foi encontrado", 'disallow');
            } else {
        ...
        if (isset($schema->not)) {
            ...
            if (count($this->getErrors()) == count($initErrors)) {
                $this->addError($path, "Correspondeu a um esquema que nao deveria", 'not');
            } else {
                $this->errors = $initErrors;
            }
        }
    ...
    protected function validateOfProperties($value, $schema, $path, $i = "")
    {
        ...
        if (isset($schema->allOf)) {
            ...
            if (!$isValid) {
                $this->addError($path, "Falha em corresponder a todos os esquemas", 'allOf');
            }
        }
        ...
        if (isset($schema->anyOf)) {
            ...
            if (!$isValid) {
                $this->addError($path, "Falha em corresponder a pelo menos um esquema", 'anyOf');
            } else {
            ...
            if ($matchedSchemas !== 1) {
                $this->addErrors(
                    array_merge(
                        ...
                        array(array(
                            'property' => $path,
                            'message' => "Falha em corresponder a exatamente um esquema",
                            'constraint' => 'oneOf',
                        ),),
                        $startErrors
                    )
            ...
            if (property_exists($value, $key)) {
                if (is_string($dependency)) {
                    ...
                    if (!property_exists($value, $dependency)) {
                        $this->addError($path, "$key depende de $dependency e $dependency nao esta presente", 'dependencies');
                    }
                } else if (is_array($dependency)) {
                    // Draft 4 must be an array - e.g. "dependencies": {"bar": ["foo"]}
                    foreach ($dependency as $d) {
                        if (!property_exists($value, $d)) {
                            $this->addError($path, "$key depende de $d e $d nao esta presente", 'dependencies');
                        }
                    }
