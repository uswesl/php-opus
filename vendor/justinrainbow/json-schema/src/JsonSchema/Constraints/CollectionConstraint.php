...
class CollectionConstraint extends Constraint
{
    ...
    public function check($value, $schema = null, $path = null, $i = null)
    {
        ...
        if (isset($schema->minItems) && count($value) < $schema->minItems) {
            ...
            $this->addError($path, "Minimo de " . $schema->minItems . " items no vetor", 'minItems', array('minItems' => $schema->minItems,));
        }
        ...
        if (isset($schema->maxItems) && count($value) > $schema->maxItems) {
            ...
            $this->addError($path, "Maximo de " . $schema->maxItems . " items no vetor", 'maxItems', array('maxItems' => $schema->maxItems,));
        }
        ...
        if (isset($schema->uniqueItems) && $schema->uniqueItems) {
            ...
            if (count(array_unique($unique)) != count($value)) {
                ...
                $this->addError($path, "Proibidos items duplicados no vetor", 'uniqueItems');
            }
        }
                ...
                if (array_key_exists($k, $schema->items)) {
                    ...
                    if (property_exists($schema, 'additionalItems')) {
                        if ($schema->additionalItems !== false) {
                            ...
                            $this->addError(
                                $path, 'O item ' . $i . '[' . $k . '] não esta definido e a definicao nao permite items adicionais', 'additionalItems', array('additionalItems' => $schema->additionalItems,));
                        }
                    } else {
